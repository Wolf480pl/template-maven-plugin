...
public class Generator {
    ...
    public static void main(String[] args) throws Exception {
        ...
        instance.generate(input_directory);
    }
    ...
    private static String readFile(File input_file) throws IOException {
        if (!input_file.exists()) {
            throw new IllegalArgumentException("Couldn't find: " + input_file);
        }
    ...
    public void clean() {
        cleanDir(this.outputRootDir);
    }

    public void generate(File inputDir) {
        scanForFiles(inputDir, this.outputRootDir);
    }

    private void processEMarkers(String content, File output_dir,
    ...
    private void processKVMarkers(String content, File output_dir,
            String file_name) throws IOException {
    ...
    protected String processReplication(String content,
        ...
        for (Map.Entry<Integer, String> entry : replicated_blocks.entrySet()) {
            ...
            for (WrapperInfo info : this.wrappers) {
                ...
                for (int j = 0; j < this.wrappers.length; j++) {
    ...
    private void scanForFiles(File input_directory, File output_directory) {
    ...
    private String simplifyPath(File file) {
        String output_string = this.outputRootDir.toString();

        String file_string = file.toString();
        return file_string.substring(output_string.length() + 1);
    }

    private void writeFile(String content, File output_file)
            throws IOException {
